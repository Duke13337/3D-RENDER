...
void updateCameraPosition() {
    // Центр вращения всегда (0,0,0) для орбитальной камеры
    ...
    glLoadIdentity();
    gluLookAt(
        x + panX, y + panY, z,     // Позиция камеры (панорамирование только смещает камеру)
        panX, panY, 0.0f,          // Центр сцены всегда (0,0,0) + панорамирование
        0.0f, 1.0f, 0.0f
    );
}
...
void handleCameraInput(const SDL_Event& event) {
    if (event.type == SDL_MOUSEBUTTONDOWN) {
        ...
        else if (event.button.button == SDL_BUTTON_RIGHT) rightMouseDown = true;
    }
        ...
        else if (event.button.button == SDL_BUTTON_RIGHT) rightMouseDown = false;
    }
        ...
        if (rightMouseDown) {
            // Орбитальное вращение вокруг центра сцены (0,0,0)
            ...
            // Панорамирование (смещение сцены)
            panX += dx * 0.05f;
            panY -= dy * 0.05f;
        }
